...
echo "do not forget to set the following ssm parameter store values:"
echo "aws ssm put-parameter --data-type text --type String \\
  --tags Key=Name,Value=ChatBotApiTogetherAIKey Key=Environment,Value=All Key=Project,Value=Research \\
  --name /research/chatbot/all/together-ai-api-token --value <KEY>"
